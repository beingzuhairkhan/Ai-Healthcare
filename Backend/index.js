...
app.post('/api/v1/symptom-checker', (req, res) => {
    ...
    const result = symptomChecker.checkSymptoms(symptom);
    if (result === 'Symptom not provided' || result === 'Symptom not found') {
        return res.status(404).json({ error: result });
    }
    const remedies = symptomChecker.getHomeRemedies(result);
